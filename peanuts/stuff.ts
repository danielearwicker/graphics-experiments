...
function geometry() {
  ...
  function line(a: Vertex, b: Vertex, c: Vertex, colour: Colour) {
    positions.push(...a, ...b);
...
function weirdThing(model: Geometry) {
  model.quadrilateral(
    [-1, 1, 1],
    [-1, -1, 1],
    [-1, -1, 0.5],
...
export function setup(canvas: HTMLCanvasElement) {
  ...
  (() => {
    const gridModel = geometry();

    const positionsBuffer = gridProgram.buffer("a_position");
    const colourBuffer = gridProgram.buffer("a_colour");

    positionsBuffer.data(new Float32Array(gridModel.positions));
    colourBuffer.data(new Uint8Array(gridModel.colours));
  })();
  ...
  function updateBuffers() {
    positionsBuffer.data(
      new Float32Array(model.positions.concat(highlights.positions))
    );
  ...
  const frameCounterLocation = gl.getUniformLocation(
    program.program,
    "u_frame"
  );
  let frameCounter = 0;
  ...
  function viewport(name: string, gl: WebGLRenderingContext) {
    ...
    function render(
      ...
      gl.drawArrays(
        gl.TRIANGLES,
        0,
        (model.positions.length + highlights.positions.length) / 3
      );
    }
  ...
  canvas.addEventListener("mousemove", (e) => {
    const newMouse = [e.offsetX, e.offsetY];
    const motion = [newMouse[0] - mouse[0], newMouse[1] - mouse[1]];
    mouse = newMouse;

    if (e.buttons & 1) {
      angle[0] += motion[1] / 100;
      angle[1] += motion[0] / 100;
    }

    const dpr = window.devicePixelRatio || 1;
    const x = mouse[0] * dpr,
      y = gl.canvas.height - mouse[1] * dpr;
  ...
  function resizeCanvasContinuously() {
    ...
    topLeft.render(0, ch, cw, ch, initial, rotateX(-Math.PI / 2), final);

    topRight.render(cw, ch, cw, ch, initial, final);

    bottomLeft.render(0, 0, cw, ch, initial, rotateY(-Math.PI / 2), final);
